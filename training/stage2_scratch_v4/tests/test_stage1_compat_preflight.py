...
def test_dry_run_executes_without_training_loop(tmp_path):
    from stage1_compat.constants import DATASET_IDENTITY_PATH
    if not DATASET_IDENTITY_PATH.is_file():
        pytest.skip("stage1_compat/dataset_identity.json excluded from Stage 2 scratch release package")
    data_dir = Path("../PlantVillage-Dataset/raw/color")
    if not data_dir.exists():
        for cand in [
            Path("../../training-data/stage2/PlantVillage-Dataset/raw/color"),
            Path("../../dataset/PlantVillage-Dataset/raw/color"),
            Path("../../train-gd-2/PlantVillage-Dataset/raw/color"),
        ]:
            if cand.exists():
                data_dir = cand
                break
