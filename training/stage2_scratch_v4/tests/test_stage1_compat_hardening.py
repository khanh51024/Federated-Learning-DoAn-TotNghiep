...
def test_lock_real_second_process_and_crash_release(tmp_path):
    ...
    code = (
        ...
        "from pathlib import Path; from stage1_compat.budget import RunnerLock; import time;"
        "lock=RunnerLock(Path(sys.argv[1])); lock.acquire(); print('LOCKED',flush=True); time.sleep(30)"
    )
    env = os.environ.copy()
    existing_pp = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = f"{repo_root}{os.pathsep}{existing_pp}" if existing_pp else repo_root
    child = subprocess.Popen([sys.executable, "-c", code, str(path)], stdout=subprocess.PIPE, text=True, env=env)
    try:
    ...
    for _ in range(50):
        try:
            ...
            time.sleep(0.05)
    assert acquired, "Lock was not released after child process termination"
...
def test_collect_entrypoint_does_not_import_runner(tmp_path):
    try:
        import stage1_compat.cli
    except ImportError:
        pytest.skip("stage1_compat.cli excluded from Stage 2 scratch scope")
...
def test_zero_quota_cli_never_starts_worker(tmp_path):
    try:
        import stage1_compat.__main__
    except ImportError:
        pytest.skip("stage1_compat.__main__ excluded from Stage 2 scratch scope")
